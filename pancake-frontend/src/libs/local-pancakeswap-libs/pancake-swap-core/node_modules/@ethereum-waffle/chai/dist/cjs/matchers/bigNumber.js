...
function overwriteBigNumberFunction(functionName, readableName, _super, chaiUtils) {
    return function (...args) {
        ...
        if (ethers_1.utils.BigNumber.isBigNumber(expected) ||
            ethers_1.utils.BigNumber.isBigNumber(actual)) {
            this.assert(ethers_1.utils.bigNumberify(expected)[functionName](actual), `Expected "${expected}" to be ${readableName} ${actual}`, `Expected "${expected}" NOT to be ${readableName} ${actual}`, expected, actual);
        }
        else {
